...
public class IMGClipWindow implements IMGClip {
    ...
    private int sizeMode;//0-原图比例，1- 1：1，2- 2：1，3- 3：2，4- 4：3

    public void reset(RectF clipImage, float rotate, int sizeMode) {
        this.sizeMode = sizeMode;
        RectF imgRect = new RectF();
        M.setRotate(rotate, clipImage.centerX(), clipImage.centerY());
        M.mapRect(imgRect, clipImage);

        float baseSize = imgRect.width() > imgRect.height() ? imgRect.height() : imgRect.width();
        switch (sizeMode) {
            case 1://1:1
                reset(baseSize, baseSize);
                break;
            case 2://2:1
                reset(baseSize, baseSize / 2);
                break;
            case 3://3:2
                reset(baseSize, baseSize / 3 * 2);
                break;
            case 4://4:3
                reset(baseSize, baseSize / 4 * 3);
                break;
            default:
                reset(imgRect.width(), imgRect.height());
        }
