...
public abstract class ActBase extends AppCompatActivity {
    ...
    public void postFinish() {
        x.task().postDelayed(() -> finish(), 300);
    }

    @Override
    public void onDestroy() {
        if (mCancelableList != null) {
            for (Callback.Cancelable req : mCancelableList) {
    ...
    public void addRequest(Callback.Cancelable req) {
        if (mCancelableList == null) mCancelableList = new ArrayList<>();
        mCancelableList.add(req);
    }
    ...
    //不需要请求参数
    public <T> void Post(AlertDialog dialog, String url, final Class<T> dataClass, IHttpCallBack<T> callBack) {
    ...
    //no dialog
    public <T> void Get(String url, RequestParams params, final Class<T> dataClass, IHttpCallBack<T> callBack) {
        Get(null, url, params, dataClass, callBack);
    }

    //no param
    public <T> void Get(AlertDialog dialog, String url, final Class<T> dataClass, IHttpCallBack<T> callBack) {
...
//        Class<T> tClass = (Class<T>) ((ParameterizedType) getClass().getGenericSuperclass()).getActualTypeArguments()[0];
    ...
    public void Put(AlertDialog dialog, String url, RequestParams params, IHttpCallBack callBack) {
        Put(dialog, url, params, Object.class, callBack);
    }
