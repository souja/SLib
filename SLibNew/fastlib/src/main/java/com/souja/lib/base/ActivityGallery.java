...
public class ActivityGallery extends ActBase {
    ...
    private void initIntent() {
        ...
        mListSelected.addAll(MTool.getUrlsLocal(ActPhotoGallery.getInstance().getSelectedPathListB()));
        if (bOnlySelected) {
            mListAll.addAll(MTool.getUrlsLocal(ActPhotoGallery.getInstance().getSelectedPathListB()));
        } else {
            mListAll.addAll(ActPhotoGallery.getInstance().getAdapterList());
        }
    ...
    private void checkEditPermission(File selectedFile) {
        ...
        mPermissionUtil.setListener(new PermissionUtil.PermissionCheckListener() {
            ...
            public void ok() {
                ...
                IMGEditActivity.open(_this,uri,editFile.getAbsolutePath(),LibConstants.FILE_PROVIDER);
            }
            ...
            public void notOk() {
                ...
                AlertDialog dialogRefused = DialogFactory.NewDialog(_this,
                        R.string.need_permission, "去开启", (d, i) -> {
                            d.dismiss();
