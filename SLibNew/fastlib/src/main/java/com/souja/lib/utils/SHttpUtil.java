...
public class SHttpUtil {
    ...
    public static String M_HTTP, VERSION = "";
    ...
    public static <T> Callback.Cancelable Request(AlertDialog dialog, String url, HttpMethod method, RequestParams mParams,
        ...
        mParams.setHeader("Connection", "close");
        addIdentify(mParams);
    ...
    public static <T> void handleOnRequestSuccess(String result, RequestParams params,
        ...
        if (code == M_HTTP_SUCCESS) {//===========请求成功（网络正常 && 参数正确）
            ODataPage pageObj;//分页模型
            if (isNull(resultObj.pagination)) pageObj = new ODataPage();
            else
                pageObj = GsonUtil.getObj(resultObj.pagination.toString(), ODataPage.class);
            ...
            if (isNull(resultObj.data)) {
                ...
                callBack.OnSuccess(msg, pageObj, null);
            } else if (resultObj.data.isJsonArray()) {
                //===========接口数据默认交互类型：JsonArray
                //===========Data Empty
                if (resultObj.data.getAsJsonArray().size() == 0) {
                    if (callBack2 != null) {
                        callBack2.handle("");
                        return;
                    }
