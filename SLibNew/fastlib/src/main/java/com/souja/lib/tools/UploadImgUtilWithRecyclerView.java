 ...
 */
public class UploadImgUtilWithRecyclerView {
    ...
    public UploadImgUtilWithRecyclerView(String uploadConfigUrl) {
        this.uploadConfigUrl = uploadConfigUrl;
    }

    public UploadImgUtilWithRecyclerView(String uploadConfigUrl, int imgColumns) {
        this.uploadConfigUrl = uploadConfigUrl;
        this.imgColumns = imgColumns;
    }

    public UploadImgUtilWithRecyclerView(String uploadConfigUrl, boolean neesImgIndex) {
        this.uploadConfigUrl = uploadConfigUrl;
        this.neesImgIndex = neesImgIndex;
    }

    public UploadImgUtilWithRecyclerView(String uploadConfigUrl, int imgColumns, boolean neesImgIndex) {
        this.uploadConfigUrl = uploadConfigUrl;
        this.imgColumns = imgColumns;
        this.neesImgIndex = neesImgIndex;
    }
    ...
    private void continueCompress(String filePath) {
        compressChildIndex++;
//        if (bCompressMarkedImg) {
//            markedImgPathCompressed.get(compressFatherIndex).add(filePath);
...
//                    bCompressMarkedImg = false;
                ...
                case 33://执行添加水印
                    ...
                    break;
             /*   case 111://添加水印后需要重新压缩一遍
//                    bCompressMarkedImg = true;
                    if (compressChildIndex >= imgPathMarked.get(compressFatherIndex).size()) {
                        LogUtil.e("第" + (compressFatherIndex + 1) + "组图片已全部压缩:");
                        compressChildIndex = 0;//重置childIndex
                        if (compressFatherIndex >= imgPathMarked.size() - 1) {
                            compressFatherIndex = 0;//重置fatherIndex
                            LogUtil.e("所有组图片都已压缩");
//                            bCompressMarkedImg = false;
                        ...
                        String imgPath = imgPathMarked.get(compressFatherIndex).get(compressChildIndex);
                        if (imgPath.contains("http:") || imgPath.contains("https:")) {
                            LogUtil.e("跳过网络图片：" + imgPath);
                            continueCompress(imgPath);
                        } else {
                    ...
                    break;
                case 222://（压缩完成 || 压缩过&&提交过&&没有修改过）=> 判断是否需要添加水印
                    LogUtil.e("水印图片压缩完成");
                    mHandler.sendEmptyMessage(44);
                    break;*/
                case 44://获取上传参数配置
