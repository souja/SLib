...
public class CircularImageView extends AppCompatImageView {
    ...
    private void init(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        if (attributes.getBoolean(R.styleable.CircularImageView_civ_border, true)) {
            setBorderWidth(attributes.getDimension(R.styleable.CircularImageView_civ_border_width, 1));
    ...
    public void setScaleType(ScaleType scaleType) {
        if (scaleType != SCALE_TYPE) {
            throw new IllegalArgumentException(MTool.formatStr("ScaleType %s not supported. ScaleType.CENTER_CROP is used by default. So you don't need to use ScaleType.", scaleType));
        }
    }
    //endregion
