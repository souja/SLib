...
//                measureChild(childView, widthMeasureSpec, heightMeasureSpec);
                ...
                if (i > 0) {//第一个布局是Left item，从第二个开始才是RightMenu
                    mRightMenuWidths += childView.getMeasuredWidth() + lp.leftMargin + lp.rightMargin;
                } else {
        ...
        for (int i = 0; i < childCount; i++) {
            ...
            if (childView.getVisibility() != GONE) {
                if (i == 0) {//第一个子View是内容 宽度设置为全屏
                    ...
                    if (isLeftSwipe) {
                        childView.layout(left + leftMargin,
                                getPaddingTop() + topMargin,
                                left + childView.getMeasuredWidth() + leftMargin,
                                getPaddingTop() + childView.getMeasuredHeight() + topMargin);
                        left = left + childView.getMeasuredWidth() + leftMargin;
                    } else {
//                        ViewGroup.LayoutParams params = childView.getLayoutParams();
                        childView.layout(right - childView.getMeasuredWidth() + leftMargin,
                                getPaddingTop() + topMargin,
                                right + leftMargin,
                                getPaddingTop() + childView.getMeasuredHeight() + topMargin);
                        right = right - childView.getMeasuredWidth() + leftMargin;
                    }
