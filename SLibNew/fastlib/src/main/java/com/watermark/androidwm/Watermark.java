    ...
    private void createWatermarkText(WatermarkText watermarkText) {
        ...
        if (watermarkText != null && backgroundImg != null) {
            if (isInvisible) {
                ...
                LogUtil.e("ori img size:" + backgroundImg.getWidth() + "&" + backgroundImg.getHeight());
                Bitmap newBitmap;
//                if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.O) {
//                    newBitmap = Bitmap.createBitmap(backgroundImg.getWidth(),
//                            backgroundImg.getHeight(), Bitmap.Config.HARDWARE);
//                } else {
                newBitmap = Bitmap.createBitmap(backgroundImg.getWidth(),
                        backgroundImg.getHeight(), Bitmap.Config.ARGB_8888);
//                }
                Canvas watermarkCanvas = new Canvas(newBitmap);
                watermarkCanvas.drawBitmap(canvasBitmap, 0, 0, null);
