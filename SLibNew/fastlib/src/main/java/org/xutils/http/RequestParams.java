...
//    private int connectTimeout = 1000 * 300; // 连接超时时间
    ...
    /*package*/ void init() throws Throwable {
        if (!MTool.isEmpty(buildUri)) return;

        if (MTool.isEmpty(uri) && getHttpRequest() == null) {
    ...
    public String getUri() {
        if (redirectHandler != null)
            return MTool.isEmpty(uri) ? buildUri : uri;
        else
            return MTool.isEmpty(buildUri) ? uri : buildUri;
    }

    public String getCacheKey() {
        if (MTool.isEmpty(buildCacheKey) && builder != null) {
            HttpRequest httpRequest = this.getHttpRequest();
            if (httpRequest != null) {
